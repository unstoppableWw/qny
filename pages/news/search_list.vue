<template>
  <view class="bg">
	  ...
	  <swiper v-if="showLargeImage" :indicator-dots="true" indicator-color="#999999" indicator-active-color="#4095E5" :current="largeImgIndex"  :duration="500" style="width: 100%;height: 100%;">
	      <!-- 创建一个可以滑动的 swiper 组件，并设置其样式 -->
	      <swiper-item  v-for="(imageItem,index) in largeImageUrl" :key="index" >
	          <!-- 使用 v-for 指令循环遍历 swiperList 数组，生成多个 swiper-item 组件 -->
	          <view  class="modal"
	            @tap="closeImage"
	          	@longtap="showContextMenu(imageItem)"
	          	@load="adjustModalSize">
	          	<movable-area scale-area>
	          		<movable-view direction="all" @scale="onScale" scale="true" scale-min="1" scale-max="4" :scale-value="scale" animation="true">
	          			<image
	          			:src="imageItem"
	          			mode="widthFix" 
	          ...
	          <!-- 显示图片，图片路径通过 item.src 绑定，并设置图片高度和圆角边框 -->
	      </swiper-item>
	  	
	  </swiper>
	  
	<view style="height: 90rpx;z-index: 1"></view>
	
	<view   class="gradient-background flex flex-column justify-start align-center" style="z-index: 100;">
      
      <!-- 中间搜索框 -->
          <view style="height: 100rpx;"></view>
          <!-- <view style="height: 88rpx;width: 100%;"
           class="flex align-center px-2   bg-white">
		   <view style="height: 58rpx;background-color: lightgrey;" class="flex flex-row align-center">
			   <view class="iconfont icon-sousuo  text-muted"
			   style="margin-left: 60rpx;font-size: 25rpx; "></view>
			   <input  style=" font-size: 25rpx;" type="text" v-model="keyword" @confirm="searchEvent"
			   placeholder="搜索"
			   placeholder-style="color: #CCCCCC;"/>
		   </view> -->
           	<view style="height: 70rpx;width: 100%;" class="flex flex-row align-center justify-center">
				<view class="flex flex-row" style="height: 70rpx;width: 80%;border-radius: 20rpx;background-color: #F8F9FA;">
					<view class="iconfont icon-sousuo  text-muted flex justify-center align-center"
					style="margin-left: 20rpx;font-size: 25rpx; "></view>
					<view class="flex justify-center align-center" style="margin-left: 30rpx;">
						<input style=" font-size: 25rpx;" type="text" v-model="keyword" @confirm="searchEvent"
						placeholder="搜索"
						placeholder-style="color: #CCCCCC;"/>
					</view>
				...
				<text class="pl-2" @click="goBack" style="color: #4095E5;margin-right: 10rpx;">取消</text>
			</view>
				  	...
				  	<view style="background-color: white;border-radius: 5%;">
				  		...
				  		<view class="post-header" style="height: 380rpx;"  @click="showAnimationAndNavigate(item2.articleId,3)">
							...
							<view style="width: 70%; display: flex; flex-direction: column;margin-left: 10rpx;">
							  ...
							  <view class="info_describe" style=" text-overflow: ellipsis;white-space: nowrap; overflow: hidden;width: 400rpx;"> <text>{{item2.content}}</text></view>
								<view class="popup_shuru">
...
export default {
 ...
 onLoad(option) {
	  ...
	  console.log(bodyData)
 	  this.$H.post('/search/search.do',bodyData)
 	     .then(response => {
 	  	// 请求成功时更新dataItems        
  ...
  methods: {
    ...
    async loadmore(index) {
      ...
      this.$H.post('/search/down_load.do',bodyData)
         .then(response => {
      	// 请求成功时更新dataItems
      var learn_article = []
      var chat_article = []
      ...
      if(item.category_id==1){
    	  ...
    	  if(this.newsList[2].list.length<response.data.data.trainArticle.total)
    			item.loadmore = "上拉加载更多"
    		else
    			item.loadmore = "已到底线"
      }
