...
<template>
  <view class="chat-message">
    <view class="avatar">
      <image :src="message.avatar" class="avatar_img"/>
    </view>
	<view class="out_view">
		<view class="card">
		  <view class="header">
		    <view class="title">{{message.type}}</view>
		    <view class="room-number">预约房间号：{{message.roomId}}</view>
		  </view>
		  <view class="description">房间名称：{{message.roomName}}</view>
		  <view class="description" style="margin-top: 10rpx;">开始时间：{{getTime(message.startTime )}}</view>
		   <view class="description" style="color: grey;">您预约的训练还有十五分钟就要开始啦，请提前进入房间做好准备~</view>
		  <view class="button-container">
		    <button class="centered-button" @click="agree(message.roomId)">进入</button>
		  </view>
		</view>
	</view>
  </view>
